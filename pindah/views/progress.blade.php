...
<body class="fixed-sn grey-skin">
  ...
  <body class="fixed-sn grey-skin">
       <main>
        <div class="container">
          <h1>Progress Project #{{$id_project}}</h1>
          <div class="row">
          <div class="col-md-4">
            <div class="card">
              <div class="card-body">
                @if ($rinci[0]->bar < 100)
                  <form action="/progress/{{$id_project}}" class="range-field" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="md-form">
                      <input type="date" name="waktu_mulai" value="">
                    </div>
                    <div class="md-form">
                      <input type="date" name="waktu_selesai" value="">
                    </div>
                    <hr>
                    <div class="md-form">
                      <textarea name="keterangan" class="form-control md-textarea" rows="8" cols="80"></textarea>
                      <label>Keterangan</label>
                    </div>
                    <label>Foto</label>
                    <div class="md-form">
                      <input type="file" name="foto" class="form-control" onchange="tampilkanPreview(this,'liat')">
                    </div>
                    <img id="liat" width="300px"><br>
                    <label>Persentase</label>
                    <div class="md-form">
                      <input type="range" name="bar" min="0" max="100">
                    </div>
                    <div id="isibar"></div>
          ...
          <div class="card">
            <div class="card-body">
              <div class="progress md-progress" style="height: 20px">
                  <div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar" style="width: {{$rinci[0]->bar}}%; height: 20px" >{{$rinci[0]->bar}}%</div>
              </div>
              <table class="table table-striped">
                ...
                <tbody>
                  @foreach ($proses as $tut)
                    <tr>
                      <td>{{$tut->tanggal}}</td>
                      <td>{{$tut->keterangan}}</td>
                      <td><img src="{{url('')}}/uploadFoto/{{$tut->foto}}" width="350"></td>
                    </tr>
                  @endforeach
                </tbody>
              </table>
	...
	<script>
        ...
        $('input[name="bar"]').on('input',function(){
          $('#isibar').html($(this).val())
        });
    </script>
